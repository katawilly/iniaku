...
if (isset($_POST["btn_admin"])) {
    ...
    if (mysqli_connect_errno()) {
        ...
        mysqli_query($connection, "ALTER TABLE settings ADD COLUMN `about_info` VARCHAR(255) AFTER layout;");
        mysqli_query($connection, "ALTER TABLE settings ADD COLUMN `ind_code` VARCHAR(255) AFTER layout;");
        mysqli_query($connection, "ALTER TABLE settings ADD COLUMN `purchase_code` VARCHAR(255) AFTER layout;");
        mysqli_query($connection, "ALTER TABLE settings ADD COLUMN `enable_paypal` int DEFAULT 0 AFTER enable_registration;");
        mysqli_query($connection, "ALTER TABLE settings ADD COLUMN `version` VARCHAR(255) DEFAULT 'v1.1' AFTER paypal_email;"); 
        
        mysqli_query($connection, "ALTER TABLE skills ADD COLUMN `orders` int DEFAULT 0 AFTER parent_id;");
        mysqli_query($connection, "ALTER TABLE experience ADD COLUMN `orders` int DEFAULT 0 AFTER parent_id;");
        ...
        mysqli_query($connection, "UPDATE settings SET version = 'v1.2' WHERE id = 1;");
        mysqli_query($connection, "UPDATE settings SET about_info = 'SW52YWxpZCBMaWNlbnNlIEtleQ==' WHERE id = 1;");


        mysqli_query($connection, "ALTER TABLE users ADD COLUMN `enable_appointment` int DEFAULT 1 AFTER enable_blog;");
        mysqli_query($connection, "ALTER TABLE users ADD COLUMN `enable_service` int DEFAULT 1 AFTER enable_blog;");


        /* Import follower table */
        $query = '';
        $sqlScript = file('sql/follower.sql');
      ...
      /* Import assaign days table */
      $query = '';
      $sqlScript = file('sql/assaign_days.sql');
      foreach ($sqlScript as $line) {
  ...
  $redir = str_replace('updates/update_v1.1_v1.2/', '', $redir);
